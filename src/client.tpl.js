...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const NEU_CSS = [
			...
			"}",
			"/* Glassmorphism: floating surfaces (menus — role=\"menu\" — and the",
			"   composer context panel — role=\"dialog\") sit INSIDE the frosted",
			"   composer card, so a backdrop-filter on them cannot blur (the card",
			"   is their backdrop root). They are instead very translucent, letting",
			"   the card's frost show through — same glass, plus a hairline top",
			"   highlight for the panel edge. */",
			"body[data-dsh-neu] [role='menu'],",
			"body[data-dsh-neu] [data-composer-card] [role='dialog'] {",
			"  background-color: color-mix(in srgb, var(--dsw-alias-bg-overlay) 30%, transparent);",
			...
			"  background-color: color-mix(in srgb, var(--dsw-alias-bg-overlay) 35%, transparent);",
			"  box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.08);",
			"}",
			"/* Composer capsule: whole-card frosted glass. The card is the backdrop",
			"   root for floats rendered inside it, so they cannot blur on their",
			"   own — instead they sit at low translucency and let THIS frost show",
			"   through, reading as the same frosted glass. */",
