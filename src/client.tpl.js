...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const NEU_CSS = [
			...
			"}",
			"/* Glassmorphism: floating surfaces (dropdown menus — role=\"menu\" — and",
			"   the composer-side context panel — role=\"dialog\" inside the composer",
			"   card) get a REAL backdrop blur now that the composer card carries no",
			"   backdrop-filter of its own (backdrop root would isolate them). */",
			...
			"}",
			"/* Composer capsule: translucent panel; the frosted look lives on the",
			"   draft scrollport (data-input-scroll), NOT on the card itself — a",
			"   card backdrop-filter would become the backdrop root for the floating",
			"   menus/dialogs rendered inside it and disable their blur. */",
			...
			"  background-color: color-mix(in srgb, var(--dsw-specific-input-major) 50%, transparent);",
			"}",
			"/* Draft scrollport: the composer's own frosted surface — blurs the",
			"   transcript scrolling behind the input text. It is a sibling of the",
			"   float anchors inside the card, so its backdrop-filter does not",
			"   isolate theirs (InputBar's data-input-scroll — verified in",
			"   ui-conversation sources). */",
			"body[data-dsh-neu] [data-input-scroll] {",
