...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const NEU_CSS = [
			...
			"}",
			"/* Glassmorphism: floating surfaces share the composer card's glass",
			"   language. They render INSIDE the card, whose backdrop-filter is",
			"   their backdrop root — a nested backdrop-filter cannot blur through",
			"   it, so the floats carry a translucent surface over the card's",
			"   frosted backdrop instead of their own blur. */",
			"body[data-dsh-neu] [role='menu'],",
			"body[data-dsh-neu] [data-composer-card] [role='dialog'] {",
			"  background-color: color-mix(in srgb, var(--dsw-alias-bg-overlay) 60%, transparent);",
			"}",
			"body[data-dsh-neu][data-ds-dark-theme] [role='menu'],",
			"body[data-dsh-neu][data-ds-dark-theme] [data-composer-card] [role='dialog'] {",
			"  background-color: color-mix(in srgb, var(--dsw-alias-bg-overlay) 66%, transparent);",
			"}",
			"/* Composer capsule: frosted glass — a translucent surface over the",
			"   scrolling transcript, blurred so message text behind it reads as",
			"   soft shapes (InputBar's data-composer-card — verified in",
			"   ui-conversation sources). */",
			"body[data-dsh-neu] [data-composer-card] {",
			"  background-color: color-mix(in srgb, var(--dsw-specific-input-major) 18%, transparent);",
			"  backdrop-filter: blur(6px) saturate(1.1);",
			"  -webkit-backdrop-filter: blur(6px) saturate(1.1);",
			"  box-shadow: var(--dsh-neu-inset);",
			"}",
			"body[data-dsh-neu][data-ds-dark-theme] [data-composer-card] {",
			"  background-color: color-mix(in srgb, var(--dsw-specific-input-major) 22%, transparent);",
			"}",
