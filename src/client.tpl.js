...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const NEU_CSS = [
			...
			"  background-color: color-mix(in srgb, var(--dsw-specific-input-major) 28%, transparent);",
			"  backdrop-filter: blur(10px) saturate(1.15);",
			"  -webkit-backdrop-filter: blur(10px) saturate(1.15);",
			"  box-shadow: var(--dsh-neu-inset);",
			"}",
			"body[data-dsh-neu][data-ds-dark-theme] [data-composer-card] {",
			"  background-color: color-mix(in srgb, var(--dsw-specific-input-major) 32%, transparent);",
			"}",
