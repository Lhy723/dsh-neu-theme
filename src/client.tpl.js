...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const NEU_CSS = [
			...
			"}",
			"/* Glassmorphism: frosted translucent floating surfaces — dropdown menus",
			"   (PermissionSelect / ModelSelect / Menu — all role=\"menu\") and the",
			"   composer-side context panel (ContextMeter — role=\"dialog\" inside the",
			"   composer card) blur the ambient light behind them. */",
			"body[data-dsh-neu] [role='menu'],",
			"body[data-dsh-neu] [data-composer-card] [role='dialog'] {",
			"  background-color: color-mix(in srgb, var(--dsw-alias-bg-overlay) 70%, transparent);",
