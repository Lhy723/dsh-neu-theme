...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const NEU_CSS = [
			"body[data-dsh-neu] {",
			"  --dsh-neu-raise: 6px 6px 14px rgba(122, 118, 108, 0.22), -6px -6px 14px rgba(255, 255, 255, 0.85);",
			"  --dsh-neu-raise-sm: 3px 3px 8px rgba(122, 118, 108, 0.18), -3px -3px 8px rgba(255, 255, 255, 0.75);",
			...
			"  --dsh-neu-raise: 6px 6px 14px rgba(0, 0, 0, 0.45), -6px -6px 14px rgba(255, 255, 255, 0.05);",
			"  --dsh-neu-raise-sm: 3px 3px 8px rgba(0, 0, 0, 0.4), -3px -3px 8px rgba(255, 255, 255, 0.04);",
			...
			"}",
			"/* User and consumed-steering bubbles: barely-raised chips",
			"   (ChatNodeSeat's data-chat-flow-kind + MessageItem's .bubble). */",
			...
			"}",
			"/* Code blocks: a recessed well (CodeBlock's stable md-code-block",
			"   class — verified in ui-primitives sources). */",
			"body[data-dsh-neu] .md-code-block {",
			"  box-shadow: var(--dsh-neu-inset);",
			"}",
			"/* Tool rows: barely-raised cards (ToolRow's data-tool — verified",
			"   in ui-tool sources). */",
			"body[data-dsh-neu] [data-tool] {",
			"  box-shadow: var(--dsh-neu-raise-sm);",
			"}",
			"/* Reasoning rows: barely-raised, hover-revealed (ReasoningRow's",
			"   data-variant='think' — verified in ui-conversation sources). */",
			...
			"  }",
			"  /* Tool rows: lift on hover (shadow only — no transform, so",
			"     internal sticky/absolute geometry stays anchored). */",
			"  body[data-dsh-neu] [data-tool] {",
			"    transition: box-shadow 160ms ease;",
			"  }",
