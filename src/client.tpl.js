...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const NEU_CSS = [
			...
			"}",
			"/* Composer capsule: a translucent panel. NOTE: it must NOT carry a",
			"   backdrop-filter — that would make it the backdrop root for the",
			"   floating menus/dialogs rendered inside it and disable their blur.",
			"   The floats get the blur; the card stays a see-through panel. */",
			"body[data-dsh-neu] [data-composer-card] {",
			"  background-color: color-mix(in srgb, var(--dsw-specific-input-major) 55%, transparent);",
			"  box-shadow: var(--dsh-neu-inset);",
			"}",
			"body[data-dsh-neu][data-ds-dark-theme] [data-composer-card] {",
			"  background-color: color-mix(in srgb, var(--dsw-specific-input-major) 60%, transparent);",
			"}",
